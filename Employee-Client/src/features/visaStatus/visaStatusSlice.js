...
export const updateDocumentStatus = createAsyncThunk('visaStatus/updateDocumentStatus',
  async ({ documentId, status }, { rejectWithValue }) => {
    try {
      const response = await customFetch.patch(`/documents/${documentId}/status`, { status });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
  ...
  async ({ documentType, file }, { rejectWithValue }) => {
    try {
      ...
      if (error.response.status === 403) {
        errorMessage = "You must wait for the previous document to be approved before uploading this document.";
