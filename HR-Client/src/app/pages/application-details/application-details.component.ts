...
export class ApplicationDetailsComponent implements OnInit {
  ...
  approveApplication(): void {
    this.updateApplicationStatusService.approveApplication(this.profile._id).subscribe({
      next: () => {
  ...
  rejectApplication(): void {
    this.updateApplicationStatusService.rejectApplication(this.profile._id).subscribe({
      next: () => {
        this.toastr.success('Application rejected.');
        this.http.put(`http://localhost:3100/hiring/addFeedback`, {
          id: this.profile._id,
